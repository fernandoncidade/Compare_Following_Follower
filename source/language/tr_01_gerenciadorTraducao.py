...
class GerenciadorTraducao(QObject):
    ...
    def _resolver_caminhos_traducao(self, codigo_idioma):
        try:
            nomes_preferidos = [
                f"tea_tdah_{codigo_idioma}.qm",
                f"followers_following_{codigo_idioma}.qm",
            ]
            caminhos: list[str] = []
            vistos: set[str] = set()

            for nome in nomes_preferidos:
                caminho = os.path.join(self.dir_traducoes, nome)
                if os.path.exists(caminho):
                    normalizado = os.path.normcase(os.path.abspath(caminho))
            ...
            alvo = codigo_idioma.lower()
            curinga = f"_{alvo}.qm"
            for nome in sorted(os.listdir(self.dir_traducoes)):
                nome_lower = nome.lower()
                if not nome_lower.endswith(".qm"):
                    continue
                if curinga not in nome_lower and not nome_lower.endswith(f"{alvo}.qm"):
                    continue
                caminho = os.path.join(self.dir_traducoes, nome)
                normalizado = os.path.normcase(os.path.abspath(caminho))
                if normalizado not in vistos:
            ...
            logger.error(f"Erro ao resolver caminhos de tradução para '{codigo_idioma}': {e}", exc_info=True)
            return []
    ...
    def aplicar_traducao(self):
        try:
            ...
            caminhos_traducao = self._resolver_caminhos_traducao(self.idioma_atual)
            caminho_carregado = None

            for caminho_traducao in caminhos_traducao:
                tradutor = QTranslator()
                if tradutor.load(caminho_traducao):
                    ...
                    caminho_carregado = caminho_traducao
                    break

            if aplicou_app_trad:
                logger.debug(f"Tradução de app carregada: {caminho_carregado}")
            else:
                logger.warning(
                    f"Nenhum arquivo .qm compatível encontrado para idioma '{self.idioma_atual}' em {self.dir_traducoes}"
                )
            ...
            logger.debug(f"Arquivos disponíveis em {self.dir_traducoes}:")
            ...
            try:
                QLocale.setDefault(QLocale(self.idioma_atual))
                logger.debug(f"QLocale padrão definido para: {self.idioma_atual}")

            except Exception as e:
                logger.debug(f"Falha ao definir QLocale padrão: {e}", exc_info=True)

            return aplicou_app_trad
